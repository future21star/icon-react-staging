...
@asyncConnect([{
	promise: ({store: {dispatch, getState}}) => {
		...
		promises.push(dispatch(markMUAsNewForm()));
...
export default class SpecialtyProgramsAssessment extends Component {
	...
	saveMUResult = () => {
		this.props.saveMU(this.props.form);
	};
	...
	render() {
		const {form, changeMUInput} = this.props;
					...
					<div className="assessment-form-wrapper container">
						<div className="container-small">
							<div className="assessment-tabs-nav row sp-assessment-tabs-nav">
								<div onClick={e => changeMUInput('evaluation', 'strength')} className={`col-xs-12 col-md-4 ${form.evaluation === 'strength' ? 'active' : ''}`}>
									<a href="javascript:;">Strength</a>
								</div>
								<div onClick={e => changeMUInput('evaluation', 'technique')} className={`col-xs-12 col-md-4 ${form.evaluation === 'technique' ? 'active' : ''}`}>
									<a href="javascript:;">Technique</a>
								</div>
								<div onClick={e => changeMUInput('evaluation', 'flexibility')} className={`col-xs-12 col-md-4 ${form.evaluation === 'flexibility' ? 'active' : ''}`}>
								...
								<div className="col-xs-12 col-md-6 pull-left pull-left-desktop">
									<div className="sp-assessment-form-wrapper">
										...
										<form className="form-select">
											<select className="form-control" value={form.q1} onChange={e => changeMUInput('q1', e.target.value)}>
						...
						<div className="bottom-padding text-center">
							<button onClick={this.saveMUResult} className="btn btn-primary btn-lg">Evaluate</button>
						</div>
					</div>
				</div>
			</ReactCSSTransitionGroup>
